...
function SpinningMetricCard({ label, value, subText, trend, index }: SpinningCardProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* FRONT FACE: Primary Metric Readout */}
        <div 
        ...
        {/* BACK FACE: Detailed Audited Statistics */}
        <div 
...
export default function HomePage() {
  ...
  // Ref hooks for smooth intersection fade transitions
  ...
  useEffect(() => {
    // 1. Inject the YouTube Iframe Player API asynchronously if not already loaded
    if (!window.YT) {
    ...
    // Poll until YT is loaded, then build player
  ...
  // Intersection Observer to smoothly trigger content reveal when scrolling
  useEffect(() => {
  ...
  return (
    <main dir={t.dir} className="min-h-screen bg-stone-50 text-neutral-950 tracking-tight transition-all duration-300">
      ...
      {/* Web Utility Typefaces and 3D Transformations CSS */}
      <style jsx global>{`
      ...
      {/* 2. LAYER: UTILITY HEADER ELEMENT */}
      ...
      {/* 3. LAYER: FULL-BLEED INTRO HERO PANEL WITH SYSTEM TIMELAPSE BACKGROUND */}
      <section id="firm" className="relative w-full min-h-[calc(100vh-60px)] flex items-center bg-neutral-950 text-white overflow-hidden border-b border-neutral-800">
        ...
        <div 
          ...
          style={{ 
            backgroundImage: `url('https://images.unsplash.com/photo-1539650116574-8efeb43e2750?q=80&w=1920&auto=format&fit=crop')` 
          }}
        />

        {/* BRIGHTENED CAIRO TIMELAPSE YOUTUBE EMBED PLAYER CELL */}
        ...
        <div className="max-w-7xl mx-auto w-full px-6 py-20 relative z-30 grid grid-cols-1 lg:grid-cols-3 gap-16 items-center">
          ...
          {/* STATS MATRIX: Composed of 3D-transforming metric card modules */}
        ...
        {/* Animated Smooth Scroll Down Indicator Chevron */}
      ...
      {/* 4. LAYER: WRAPPED SOLUTIONS AND MEDIA GRIDS (with smooth fade-in observer) */}
      <div 
      ...
      >
        ...
        {/* SOLUTIONS PORTFOLIO SUBSECTION */}
        <section id="services" className="w-full overflow-hidden">
          ...
          <div className="relative w-full">
            ...
            >
              {t.businessLines[activeTab].map((subSection: any) => (
                <div 
                  key={subSection.id} 
                  className="min-w-[85%] sm:min-w-[45%] lg:min-w-[31%] snap-start bg-white rounded-none border border-neutral-200 shadow-sm flex flex-col overflow-hidden hover:shadow-md transition-all duration-200 border-t-2 hover:border-t-neutral-900"
                >
                  ...
                  <div className="p-8 py-12 flex-1 flex flex-col justify-between gap-8">
                    <div>
                      ...
                      <p className="text-neutral-500 text-sm leading-relaxed font-light line-clamp-4">{subSection.desc}</p>
                    </div>
            ...
            <div className={`flex mt-4 ${t.dir === 'rtl' ? 'justify-start' : 'justify-end'}`}>
              <div dir="ltr" className="flex items-center gap-2 bg-white border border-neutral-200 p-1 rounded-none shadow-sm">
                <button 
                  ...
                  className="w-9 h-9 flex items-center justify-center rounded-none hover:bg-slate-50 text-slate-600 hover:text-neutral-950 active:scale-95 transition-all cursor-pointer text-sm font-bold"
                >
                  ←
                </button>
